...
# Extracting BC Canada Data
 # Filter data for Sri Lanka and select relevant columns
 bc_canada_data <- real_data %>%
   filter(Province.State == "British Columbia", Country.Region == "Canada") %>%
   dplyr::select(Date, Confirmed) %>%
   arrange(Date)  # Sorting the data by date
 
 # Create a new column for daily new confirmed cases
 bc_canada_data <- bc_canada_data %>%
   arrange(Date) %>%  # Make sure data is in date order
   mutate(Previous_Confirmed = lag(Confirmed, 1, default = 0),  # Shift confirmed cases by one row
          New_Confirmed = Confirmed - Previous_Confirmed)

# Remove the first 20 rows using base R
bc_canada_data <- bc_canada_data[-(1:37), ] 
 
 # Convert the Date column to Date format if it's not already
 bc_canada_data$Date <- as.Date(bc_canada_data$Date, format = "%Y-%m-%d")
 
 # Check the structure of the dataframe to ensure Date is in Date format and Confirmed cases are numbers
 str(bc_canada_data)

 # Compute the 'times' column
 bc_canada_data <- bc_canada_data %>%
   arrange(Date) %>%  # Make sure data is in chronological order
   mutate(times = Date - min(Date))  # Subtract the first date from each date in the dataset
 
 # To ensure 'times' is in days as integers
 bc_canada_data <- bc_canada_data %>%
   mutate(times = as.integer(times))
 
 # Real Data Analysis for BC Canada Data
 
 data <- bc_canada_data[, (4:5)]
 ...
 prior <- function(theta){
   ...
   log_prior_beta <- dlnorm(beta, log(0.145), 0.2, log = T)  
   log_prior_delta <- dlnorm(delta, log(1/5), 0.2, log = T) 
   log_prior_gamma <- dlnorm(gamma, log(1/7), 0.2, log = T)  
   
   return(log_prior_beta + log_prior_delta + log_prior_gamma) 
 }
 
 reference <- prior
 N <- 5100000 # Approximately 5.1 million BC population
 K <-1000
 d <- 3
 init <- function(){c(rlnorm(1, log(0.145), 0.2), 
                      rlnorm(1, log(1/5), 0.2),
                      rlnorm(1, log(1/7), 0.2))} 
...
asmc_output <- asmc(K, theta_cov, tuning_param, init, data, likelihood, prior, reference)
 ...
 BC_ASMC_AMTM_rsp <- unlist(resampled_particles)
 ...
 BC_ASMC_AMTM_ESS <- data.frame(ESS = ess_values)
 BC_ASMC_AMTM_alpha <- data.frame(alpha_val = alpha_values)
 BC_ASMC_AMTM_W <- data.frame(W = W_values)
 BC_ASMC_AMTM_logZ <- data.frame(logZ = logZ_values)
 
 write.table(BC_ASMC_AMTM_ESS, file = "/scratch/theniw8/BC_ASMC_AMTM_Finale_1000_0.99_ESS.csv", sep = "," ,row.names = FALSE)
 write.table(BC_ASMC_AMTM_alpha, file = "/scratch/theniw8/BC_ASMC_AMTM_Finale_1000_0.99_alpha.csv", sep = "," ,row.names = FALSE)
 write.table(BC_ASMC_AMTM_W, file = "/scratch/theniw8/BC_ASMC_AMTM_Finale_1000_0.99_W.csv", sep = "," ,row.names = FALSE)
 write.table(BC_ASMC_AMTM_logZ, file = "/scratch/theniw8/BC_ASMC_AMTM_Finale_1000_0.99_logZ.csv", sep = "," ,row.names = FALSE)
 write.table(BC_ASMC_AMTM_rsp, file = "/scratch/theniw8/BC_ASMC_AMTM_Finale_1000_0.99_rsp.csv", sep = "," )
