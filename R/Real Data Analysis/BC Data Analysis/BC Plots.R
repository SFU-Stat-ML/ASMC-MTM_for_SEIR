setwd("C://SFU//Research//Comparison//AdaptiveASMC//Real Data//Real Data Analysis//BC Analysis")
...
resampled_particles1 <- read.csv("BC_ASMC_AMTM_Finale_1000_0.99_rsp.csv")
...
resampled_particles1 <- resampled_particles1 %>%
  dplyr::rename(Beta = V1, Delta = V2, Gamma = V3)
...
ggsave("BC_ASMC_AMTM_DEN.jpeg", plot = p1, width = 12, height = 8, dpi = 300)

par(mfrow = c(1,2))
ess7 <- read.csv("BC_ASMC_AMTM_Finale_1000_0.99_ESS.csv")
alpha7 <- read.csv("BC_ASMC_AMTM_Finale_1000_0.99_alpha.csv")

ess7 <- ess7 %>%
  mutate(Iteration = 1:45)

ess7 <- ess7 %>%
  dplyr::select(Iteration, everything())

alpha7 <- alpha7 %>%
  mutate(Iteration = 1:46)
...
# Save the plot with high resolution
ggsave("BC_ASMC_AMTM_SEIR_ESS_Alpha.jpeg", plot = cplot1, width = 12, height = 8, dpi = 300)
