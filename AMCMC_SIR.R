...
for (j in 1:n_chains){
  ...
  for (i in 1:n_iter) {
    if (i < n_iter0){
      v0 <- abs(current_params)
      params_sd <- 0.1 * v0
      proposed_params <- rnorm(d, current_params, params_sd/d)
    }
    else{
      v_i <- colMeans(samples_matrix[1:i, ])
      if (runif(1) < b){
        proposed_params <- rnorm(d, current_params, 0.1 * v_i/ d)
      }
      else{
        proposed_params <- rmvnorm(1, current_params, (2.38^2 / d) * Sigma)
      }
    }
